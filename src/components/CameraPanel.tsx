  ...
  return (
    ...
    >
            ...
            {onChangeSpeed && (
              <div className="flex items-center bg-slate-950/80 rounded-lg p-0.5 border border-slate-800">
                <button
                  type="button"
                  id="speed-ultra-btn"
                  onClick={() => onChangeSpeed(450)}
                  title="Ultra-Fast: 450ms sampling interval for rapid sign-to-text"
                  className={`px-2 py-0.5 rounded text-[11px] font-semibold transition-colors flex items-center gap-1 cursor-pointer ${
                    settings.sampleIntervalMs <= 550
                ...
                <button
                  type="button"
                  id="speed-fast-btn"
                  onClick={() => onChangeSpeed(750)}
                  title="Fast: 750ms sampling interval (snappy & responsive)"
                  className={`px-2 py-0.5 rounded text-[11px] font-semibold transition-colors flex items-center gap-1 cursor-pointer ${
                    settings.sampleIntervalMs > 550 && settings.sampleIntervalMs <= 950
      ...
      <div className="relative aspect-[4/3] sm:aspect-video w-full bg-black flex items-center justify-center overflow-hidden">
        ...
        {/* Real-time Vision Tracking & Landmark Overlay Canvas */}
        {cameraPermission === 'granted' && overlayCanvasRef && (
          <canvas
            ref={overlayCanvasRef as React.RefObject<HTMLCanvasElement>}
            id="camera-telemetry-overlay-canvas"
            className={`absolute inset-0 w-full h-full object-cover pointer-events-none z-15 transition-opacity duration-200 ${
              settings.mirrored ? 'scale-x-[-1]' : 'scale-x-100'
            } ${showDebugOverlay ? 'opacity-100' : 'opacity-0'}`}
          />
        )}
        ...
        {/* Floating Top-Right Turn Off Camera Button */}
        {cameraPermission === 'granted' && (
          <div className="absolute top-4 right-4 z-20">
            <button
              id="camera-panel-overlay-stop-btn"
              onClick={onStopCameraAndTranslation}
              className="px-3 py-1.5 rounded-full bg-slate-950/80 hover:bg-rose-950/90 border border-slate-700/80 hover:border-rose-500/60 text-slate-200 hover:text-rose-200 text-xs font-semibold backdrop-blur-md flex items-center gap-1.5 shadow-lg transition-all cursor-pointer"
          ...
          {cameraPermission === 'granted' ? (
            <>
              ...
              <button
                ...
                onClick={onStopCameraAndTranslation}
                className="px-3.5 py-2 rounded-xl bg-slate-900 hover:bg-rose-950/90 border border-slate-800 hover:border-rose-500/50 text-slate-300 hover:text-rose-200 font-bold text-xs transition-all flex items-center gap-1.5 cursor-pointer shadow-sm"
                title="Turn off video camera completely"
              >
                <CameraOff className="w-3.5 h-3.5 text-rose-400" />
                <span>Turn Off Camera</span>
              </button>
